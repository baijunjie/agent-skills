...
- 产生代码变更的任务，一律在独立的 git worktree + 独立分支上完成，编辑、构建、测试、质量检查都在里面进行；主工作副本保持干净，便于随时对照目标分支。只读排查、答疑这类不产生改动的操作除外。只为验证而临时注入代码时，可另开不建分支的临时 worktree（`git worktree add --detach`），里面的改动不提交、验完即删。
- 目标分支是创建 worktree 时主工作副本所在的分支，不一定是主分支；开发分支从它的最新提交拉出，最终也合并回它。创建时必须记下目标分支和切出点：`git config branch.<分支>.worktreeTarget <目标分支>`、`git config branch.<分支>.worktreeBase $(git rev-parse <目标分支>)`——换了会话也查得到该合并回哪；回退闸门靠前者守护目标分支，靠后者把检查范围覆盖到切出点之后的全部提交。之后 rebase 也不要改切出点，删除分支时这两条会随之清掉。
